...
const Header = () => {
	...
	if (role?.host) {
		navLinks.splice(3, 0, { label: "Dashboard", href: "/dashboard" });
	} else if (role?.moderator) {
		navLinks.splice(3, 0, { label: "Request", href: "/request" });
	}
		...
		<header className="p-5 z-10 max-w-[1440px] mx-auto text-lavender_bush">
			<nav className="flex justify-between items-center">
				...
				<div className="hidden max-lg:block">
					<button className="text-white text-lg" onClick={() => setMenuOpen(!menuOpen)}>
						☰
					</button>
				</div>
			</nav>
						...
						<li key={item?.label} className="text-lg w-full">
							<Link
								...
								className={`capitalize hover:text-purple-500 ${
									location?.pathname === item?.href ? "text-purple-500 font-bold" : ""
								}`}
